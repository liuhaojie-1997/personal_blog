...
class User(AbstractUser):
    ...
    email=models.EmailField(validators=[RegexValidator(r'^[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$','邮箱格式不正确'),]
                            )
    # gender=models.PositiveIntegerField(choices=GENDER_CHOICE,
                                       # default=DEFAULT_GENDER)

    # gender=models.PositiveIntegerField()
    gender=models.CharField(verbose_name='性别',max_length=20,choices=(('male','男'),('female','女'),('unknown','保密')),default='unknown')
    hobby=models.TextField(max_length=32,null=True,blank=True)
...
class ArticleManager(models.Manager):
    def distinct_date(self):
        ...
        for date in date_list:
            date = date['date_publish'].strftime('%Y/%m{m}').format(m='文章存档')
